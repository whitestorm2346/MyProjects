...
template <typename Type> Type fn_Partition(Type* typptr_Arr, int int_LowIdx, int int_HighIdx);
template <typename Type> void fn_Hoare(Type* typptr_Arr, int int_LowIdx, int int_HighIdx);
...
template <typename Type>
    Type fn_Partition(Type* typptr_Arr, int int_LowIdx, int int_HighIdx)
    {
        ...
        for(int int_Idx2 = int_HighIdx; int_Idx2 > int_Idx1; int_Idx2--)
        {
            if(typptr_Arr[int_Idx1] > typ_Pivot) int_Idx1++;

            if(typptr_Arr[int_Idx2] > typ_Pivot)
            {
                Type typ_Temp = typptr_Arr[int_Idx2];
                typptr_Arr[int_Idx2] = typptr_Arr[int_Idx1];
                typptr_Arr[int_Idx1] = typ_Temp;
            }
        }

        Type typ_Temp = typptr_Arr[int_LowIdx];
        typptr_Arr[int_LowIdx] = typptr_Arr[int_Idx1 - 1];
...
template <typename Type>
    void fn_Hoare(Type* typptr_Arr, int int_LowIdx, int int_HighIdx)
    {
        if(int_LowIdx < int_HighIdx)
        {
            ...
            fn_Hoare(typptr_Arr, int_PivotIdx + 1, int_HighIdx);
        }
